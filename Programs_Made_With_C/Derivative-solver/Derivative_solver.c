...
static Node *parse_base(void) {
    ...
    if (strncmp(pos, "sin", 3) == 0) { pos += 3; pos++; /* '(' */
        Node *arg = parse_expr(); pos++; /* ')' */
        return make_unary(NODE_SIN, arg);
    }
    if (strncmp(pos, "cos", 3) == 0) { pos += 3; pos++;
        Node *arg = parse_expr(); pos++;
        return make_unary(NODE_COS, arg);
    }
    if (strncmp(pos, "ln",  2) == 0) { pos += 2; pos++;
        Node *arg = parse_expr(); pos++;
        return make_unary(NODE_LN, arg);
    }
    if (strncmp(pos, "exp", 3) == 0) { pos += 3; pos++;
        Node *arg = parse_expr(); pos++;
        return make_unary(NODE_EXP, arg);
    }

    /* ── variable x ── */
    if (*pos == 'x') { pos++; return make_var(); }
...
static Node *simplify(Node *n) {
    ...
    if (n->left && n->right &&
        ...
        int fold = 1;
        switch (n->type) {
        case NODE_ADD: result = a + b; break;
        case NODE_SUB: result = a - b; break;
        case NODE_MUL: result = a * b; break;
        case NODE_DIV: result = (b != 0) ? a / b : 0; break;
        case NODE_POW: result = pow(a, b); break;
        default: fold = 0;
        }
        if (fold) {
            free_tree(n->left); free_tree(n->right);
            n->type = NODE_NUM; n->value = result;
            n->left = n->right = NULL;
            return n;
        }
    }
    ...
    switch (n->type) {
    case NODE_ADD:
        if (is_num(n->left,  0)) { Node *r = n->right; n->right = NULL; free_tree(n); return r; }
        if (is_num(n->right, 0)) { Node *l = n->left;  n->left  = NULL; free_tree(n); return l; }
        break;
    case NODE_SUB:
        if (is_num(n->right, 0)) { Node *l = n->left; n->left = NULL; free_tree(n); return l; }
    ...
    case NODE_MUL:
        if (is_num(n->left,  0) || is_num(n->right, 0)) {
            free_tree(n); return make_num(0);
        }
        if (is_num(n->left,  1)) { Node *r = n->right; n->right = NULL; free_tree(n); return r; }
        if (is_num(n->right, 1)) { Node *l = n->left;  n->left  = NULL; free_tree(n); return l; }
        break;
    case NODE_DIV:
        if (is_num(n->right, 1)) { Node *l = n->left; n->left = NULL; free_tree(n); return l; }
        if (is_num(n->left,  0)) { free_tree(n); return make_num(0); }
        break;
    case NODE_POW:
        if (is_num(n->right, 0)) { free_tree(n); return make_num(1); }
        if (is_num(n->right, 1)) { Node *l = n->left; n->left = NULL; free_tree(n); return l; }
        break;
    default: break;
    }
...
static void print_node(const Node *n) {
    ...
    case NODE_NUM:
        ...
        if (n->value == (long long)n->value)
            ...
            printf("%g", n->value);
        break;
    case NODE_VAR: printf("x"); break;
    case NODE_ADD:
        printf("("); print_node(n->left); printf(" + "); print_node(n->right); printf(")");
        break;
    case NODE_SUB:
        printf("("); print_node(n->left); printf(" - "); print_node(n->right); printf(")");
        break;
    ...
    case NODE_POW:
        printf("("); print_node(n->left); printf("^"); print_node(n->right); printf(")");
        break;
    case NODE_SIN:  printf("sin(");  print_node(n->left); printf(")"); break;
    case NODE_COS:  printf("cos(");  print_node(n->left); printf(")"); break;
    case NODE_LN:   printf("ln(");   print_node(n->left); printf(")"); break;
    case NODE_EXP:  printf("exp(");  print_node(n->left); printf(")"); break;
    }
}
...
static double evaluate(const Node *n, double x_val) {
    ...
    case NODE_ADD: return evaluate(n->left, x_val) + evaluate(n->right, x_val);
    case NODE_SUB: return evaluate(n->left, x_val) - evaluate(n->right, x_val);
    case NODE_MUL: return evaluate(n->left, x_val) * evaluate(n->right, x_val);
    case NODE_DIV: { double d = evaluate(n->right, x_val);
                     return d != 0 ? evaluate(n->left, x_val) / d : 0; }
    case NODE_POW: return pow(evaluate(n->left, x_val), evaluate(n->right, x_val));
    case NODE_SIN: return sin(evaluate(n->left, x_val));
    case NODE_COS: return cos(evaluate(n->left, x_val));
    case NODE_LN:  { double a = evaluate(n->left, x_val);
                     return a > 0 ? log(a) : 0; }
    case NODE_EXP: return exp(evaluate(n->left, x_val));
    default: return 0;
    }
}
